...
class TestBlocky402Live:
    async def test_get_supported_returns_hedera_testnet(self, testnet_env):
        ...
        try:
            ...
            hedera_kinds = [
                k for k in supported.kinds if k.network == "hedera:testnet" and k.scheme == "exact"
            ]
            assert len(hedera_kinds) >= 1, (
                f"Blocky402 testnet did not advertise hedera:testnet/exact. Got: {supported.kinds}"
            )
