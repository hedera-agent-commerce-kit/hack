...
from hack_pay.providers.base import VerifyResult, SettleResult
...
class TestGateErrors:
    async def test_verify_failure_returns_error(
        ...
        mock_provider.verify = AsyncMock(
            return_value=VerifyResult.failure("bad sig")
        )
    ...
    async def test_settle_failure_returns_error(
        ...
        mock_provider.settle = AsyncMock(
            return_value=SettleResult.failure("network down")
        )
        ctx = RequestContext(endpoint="/test", method="GET", payment_signature=_sig(b"tx2"))
        result = await payment_gate.gate(ctx, valid_payment_config)
        assert result.kind == "error"

    async def test_malformed_signature_returns_error(
        self, payment_gate, valid_payment_config
    ):
        ctx = RequestContext(endpoint="/test", method="GET",
                             payment_signature="not-valid-base64!!!")
        result = await payment_gate.gate(ctx, valid_payment_config)
        assert result.kind == "error"

    async def test_wrong_network_returns_error(
        self, payment_gate, valid_payment_config
    ):
        data = {"x402_version": 2, "scheme": "exact",
    ...
    async def test_error_result_never_raises(
        self, payment_gate, valid_payment_config
    ):
        # Verify that even a catastrophic provider crash returns ErrorResult
        from hack_pay.errors import FacilitatorError
        mock_provider = AsyncMock()
        mock_provider.build_payment_requirements = AsyncMock(
            side_effect=FacilitatorError("boom")
        )
