...
def make_receipt(tx_id: str = "0.0.1@100.0") -> PaymentReceipt:
    return PaymentReceipt(
        transaction_id=tx_id, payer_account_id=None,
        receiver_account_id="0.0.12345", amount_tinybars=50_000_000,
        asset="0.0.0", network="hedera:testnet",
...
class TestInMemoryIdempotencyStore:
    ...
    async def test_concurrent_put_if_absent_exactly_one_winner(self):
        ...
        results = await asyncio.gather(
            *[store.put_if_absent("same_key", r) for r in receipts]
        )
