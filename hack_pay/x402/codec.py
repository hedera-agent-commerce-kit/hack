"""
hack_pay.x402.codec Ã¢â‚¬â€ Encode and decode x402 v2 HTTP headers.
...
# 64 KB Ã¢â‚¬â€ enforced before any decode to block decompression-bomb payloads.
_MAX_HEADER_BYTES: int = 65_536


def encode_payment_required(req: PaymentRequirements) -> str:
    """Serialise PaymentRequirements Ã¢â€ â€™ Base64(UTF-8 JSON) for PAYMENT-REQUIRED header."""
    return base64.b64encode(req.model_dump_json(by_alias=True).encode()).decode()
...
def decode_payment_signature(header_value: str) -> PaymentPayload:
    ...
    try:
        ...
        raise MalformedPaymentError(f"PAYMENT-SIGNATURE is not valid Base64: {exc}") from exc

    try:
        data = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise MalformedPaymentError(f"PAYMENT-SIGNATURE decoded to invalid JSON: {exc}") from exc
    ...
    _version_keys = {
        k: v for k, v in [("x402Version", _v_camel), ("x402_version", _v_snake)] if v is not None
    }  # noqa: E501

    if _version_keys:
        for _key, _val in _version_keys.items():
            if not isinstance(_val, int) or isinstance(_val, bool):
                raise MalformedPaymentError(f"{_key} must be an exact integer, got {_val!r}")
    ...
    try:
        ...
        raise MalformedPaymentError(f"PAYMENT-SIGNATURE has invalid structure: {exc}") from exc
...
def extract_payment_signature_header(headers: dict[str, str]) -> str | None:
    ...
    if lower.get("x-payment"):
        raise UnsupportedProtocolVersionError(
            "x402 v1 header 'X-PAYMENT' is not supported. Use 'PAYMENT-SIGNATURE' (x402 v2)."
        )
